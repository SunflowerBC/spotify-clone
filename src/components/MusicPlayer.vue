...
<template>
  <div
    id="MusicPlayer"
    v-if="audio"
    class="fixed flex items-center justify-between bottom-0 w-full z-50 h-[90px] bg-teal-600 border-t border-t-gray-200"
  >
  <div class="flex items-center w-1/3">
    <div class="flex items-center ml-4">
      <img class="rounded-md shadow-2xl" width="55" :src="currentArtist.albumCover" alt="">
      <div class="ml-4">
          <div class="text-[14px] text-white hover:underline cursor-pointer">
            {{ currentTrack.name }}
          </div>
          <div class="text-[11px] text-teal-300 hover:underline hover:text-white cursor-pointer">
            {{ currentTrack.name }}
          </div>
      </div>
    </div>
    <div class="flex items-center ml-8">
    <Heart fillColor="#DB7093" :size="20"/>
